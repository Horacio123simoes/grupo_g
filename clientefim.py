...
def tocar_som(tipo):
    if tipo == 'enviar':
        winsound.PlaySound('enviar.wav', winsound.SND_ASYNC)
    elif tipo == 'receber':
        winsound.PlaySound('receber.wav', winsound.SND_ASYNC)
...
def selecionar_foto():
    ...
    caminho = filedialog.askopenfilename(filetypes=[("Imagens", "*.png *.jpg *.jpng")])
    if caminho:
        imagem = Image.open(caminho)
        imagem = imagem.resize((15, 15))
...
imagem_label = Label(windows, width=30, height=30, bg=cor5)
imagem_label.place(x=30, y=10)
